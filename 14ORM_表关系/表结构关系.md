...
class User(models.Model):
    ...
    dp = models.ForeignKey(to='Depart',to_field='id',verbose_name='部门',related_name='user',on_delete=models.CASCADE)
    # 这个和之前学的flask中的作用相似，譬如我们想通过user找到这个user对应的部门名称的时候   
    # 就可以user.dp.title ,这样跨表就拿到了我们想要的部门的名称
class Role(models.Model):
    title = models.CharField(verbose_name='标题',max_length=32)
```  
> models.ForeignKey中的几个参数有如下含义  
> to 表示关联的是哪个表
> to_field 表示的是对应的是关联的表的哪个字段，如果不写那么就是默认关联表的主键
> realated_name表示反向调用名,如上面的意思中就是如果通过部门表查询User,就可以利用realted_name来反向查找
