...
## process_view是在源码中写死的

> process_view的源码没法查看,所以我们得从wsgi.py
>

## 其它

- process_exception ,这跟之前学习scrapy的middleware时非常相似  
  当视图函数发生异常时它会捕获异常
...
- process_template_response(self,request,response)  
  它会在视图函数中,返回的是templatesreponse的对象的时候才会被执行

要想使用它必须用TemplateResponse来返回模板
...
其实就像是把之前的一步拆成了两步
以前是直接views.py里头写好return render(request,'xxx.html')

而现在是引入TemplateResponse()
先进行TemplateResponse()
然后中间件拿到它之后再进行response.render()
...
##  总结   
一般在定义中间件的时候,它是一个类  
定义的时候使用集成MiddlewareMixin的方式来做   
不建议使用之前的原始方法来进行中间件的定义

- 最常用的就是process_request,process_response,process_view

- process_exception是视图函数出现异常时会触发,它可以让我们自定义异常页面    
- process_template_response,是当我们的视图函数返回的是TemplateResponse对象或者返回的对象中有render方法的时候中间件中的它就会被触发


## 中间件具体的应用场景  
执行所有请求之前和之后要做操作的时候就使用process_request,process_response  
例如,登陆成功后需要保存凭证，下次登录前先校验

还有就是跨域请求,例如自己做了个小网站,然后开发“其它登陆方式”
譬如用qq登陆的api,浏览器有同源策略,它会认为从tencent.api返回的数据是
“非法”的,便会阻止它  
如果我们想让浏览器不限制tencent.api,那么我们就需要让process_response在响应的数据里面加上一个响应头  
然后让浏览器认为这个东西是我们自己网站的东西,这样就方便很多  
所以这样就可以绕过浏览器的跨域限制,这时候就会发现process_response非常适合
